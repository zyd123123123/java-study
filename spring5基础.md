...
![img](https://imgconvert.csdnimg.cn/aHR0cHM6Ly9naXRlZS5jb20vd29fYmVsbC9QaWN0dXJlQmVkL3Jhdy9tYXN0ZXIvaW1hZ2UvMjAyMDA4MDEwMjUxMzkucG5n?x-oss-process=image/format,png)Spirng是一个开源的免费框架（容器）Spring生活一个轻量级、非入侵的框架控制反转（IOC），面向切面编程（AOP）支持事务的处理，对框架整合的支持总结：Spring是一个轻量级的控制反转（IOC）和面向切面编程（AOP）的框架1.3、

![image-20201207154051710](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20201207154051710.png)
...
- @Autowired是先byteType,如果唯一則注入，否则byName查找。resource是先byname,不符合再继续byType，而且必须要求这个对象存在！【常用】
...
```class ProxyFactory{
//把InvocationHandler实现类封装
class ProxyFactory{
    public static Object getProxyInstance(Object obj){
        ProxyInvocationHandler Pih = new ProxyInvocationHandler();
        Pih.setTarget(obj);
        return  Proxy.newProxyInstance(obj.getClass().getClassLoader(), obj.getClass().getInterfaces(),Pih);
    }
...
```java
//不需要new invocationhandler实现类
public class Client {
    public static void main(String[] args) {
        HostMaster hostMaster = new HostMaster();
        Host proxyInstance = (Host) ProxyFactory.getProxyInstance(hostMaster);
        proxyInstance.rent();
    }
}
```
...
==固定、专注于dao操作，import到applicationContext中==
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <bean id="sqlSessionFactory" class="org.mybatis.spring.SqlSessionFactoryBean">
        ...
        <property name="mapperLocations" value="classpath:com/zyd/dao/*.xml"/>
    </bean>
...
```
 ...
 只需要改这行 <aop:pointcut id="txpointcut" expression="execution(* mapper.*.*(..))"/>
